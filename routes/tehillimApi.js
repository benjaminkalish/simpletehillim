...
(async () => {
  const tehillimArr = JSON.parse(
    await fs.readFile(path.join(__dirname, "..", "psalms.json"))
  );
  const text = tehillimArr.text;
  tehillim = text.map((x, i) => {
    return tehillimObject(x, i);
  });
})();
...
router.get("/month/:num", function (req, res, next) {
  ...
  if (day < 1 || day > 30) {
    ...
    if (day === 25) {
      ...
      res.json(
        tehillim.filter(
          (x) => x.dayMonth === day || (x.dayMonth === 30 && isChaseir)
        )
      );
    } else {
