...
export default function TrippleText({ text, font }) {
    ...
    return (
        <div id='textContainer' /* onScroll={e => e.target.scrollTo(0, 0)} */ ref={textContainerRef}>
            {<div id='innerTextContainer' ref={innerTextContainerRef} style={{ fontFamily: font}}>
                {/* displayText.slice(visibleText[0], visibleText[1]) */}
            </div>}
            <div ref={a} className='innerTextContainer'>a</div>
            <div ref={b} className='innerTextContainer'>b</div>
            <div ref={c} className='innerTextContainer'>c</div>
        </div>
    )
}
