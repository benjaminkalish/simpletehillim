        ...
        <main id='home'>
            <div>
                ...
                <div id='buttonContainer'>
                    ...
                    <a className='buttonLink' href={`/month/${hebDay.getDate()}`} onClick={(e) => navigateTo(e, `/month/${hebDay.getDate()}`)}><button>Tehillim for {hebDay.renderGematriya().split(' ').slice(0, -1).join(' ')/* .replaceAll('׳', '') */}</button></a>
                    <a className='buttonLink' href={`/week/${date.getDay() + 1}`} onClick={(e) => navigateTo(e, `/week/${date.getDay() + 1}`)}><button>Tehillim for יום {gematriya(date.getDay() + 1)}</button></a>
                </div>
                <small></small>
            </div>
            <section>
