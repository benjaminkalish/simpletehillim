...
export default function Reader({ type }) {
  ...
  const fontList = Object.entries(fonts).map((x) => (
    <li key={x[0]}>
      {" "}
      <button
        className={font === x[1] ? "selectedFont" : ""}
        onClick={() => updateFont(x[1])}
      >
        {x[0].replace("_", " ")}
      </button>
    </li>
  ...
  const [fontSizeCoefficient, setFontSizeCoefficient] = useState(
    +localStorage.fontSizeCoefficient || 1
  );
  ...
  useEffect(() => {
    ...
    const prevButton = !isFirst && (
      <Link className="navButton" key="prev" to={`/${type}/${currentNum - 1}`}>
        Previous {typeText}
      </Link>
    );
    const nextButton = !isLast && (
      <Link className="navButton" key="next" to={`/${type}/${currentNum + 1}`}>
        Next {typeText}
      </Link>
    );
    ...
    setFormattedText(
      [
        ...
        text.flatMap((datum) =>
          [
            <h2 className="perekHeading" key={datum.perek}>
              {perekName(datum.perek)}
            </h2>,
          ].concat(
            datum.text
              .join(" ")
  ...
  return (
    <>
      <div id="readerContainer">
        <button id="menuButton" onClick={menuButtonHandler}>
        ...
        <section id="menu">
          <ul>
            ...
            <li>
              <button onClick={() => setFontShown(!fontShown)}>
                Select Font
              </button>
            ...
            </li>
            {
              <li>
                ...
                {showFontSize && (
                  <div id="fontSize">
                    <button onClick={() => upDateFontSizeCoefficient(95)}>
                      &#65293;
                    </button>
                    <button onClick={() => upDateFontSizeCoefficient(105)}>
                      &#65291;
                    </button>
                  </div>
                )}
